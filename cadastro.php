...
<?php
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        function nenhumUsuario($c, $e, $s) {
            $q = mysqli_query($c, "SELECT email, senha FROM usuarios WHERE email = '$e' AND senha = md5('$s');") or die('Erro no banco de dados.');
        ...
        if (validaUsuario($email, $senha) && nenhumUsuario($c, $email, $senha)) {
            $insert = mysqli_query($c, "INSERT INTO usuarios(nome, email, senha) VALUES ('$nome', '$email', md5('$senha'));") or die('Erro no banco de dados.');
